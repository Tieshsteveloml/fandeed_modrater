	...
	CKEDITOR.plugins.add( 'cloudservices', {
		...
		beforeInit: function( editor ) {
			...
			editor.on( 'fileUploadResponse', function( evt ) {
				...
				evt.data.response = response.url;

				//GET Iframe
				var iframe = document.getElementsByClassName('cke_wysiwyg_frame cke_reset');
				var figures = iframe[0].contentDocument.getElementsByClassName('easyimage cke_widget_element easyimage-full');

				for (var i = 0; i < figures.length; i++){
					
					if(figures[i].children[0].src.indexOf("media") === -1){
					figures[i].children[0].src = response.url
					break 
				}
